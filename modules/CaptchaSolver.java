...
public class CaptchaSolver extends Module {
    ...
    try {
        // For container slots, the slot number IS the slot ID
        // Use slotId() directly, not slot()
        switch (clickType) {
            case LEFT_CLICK, PICKUP -> {
                InvUtils.click().slotId(slot);
            }
            case SHIFT_CLICK, QUICK_MOVE -> {
    ...
    private boolean isCaptchaScreen() {
        ...
        int containerSize = getContainerSize();
        if (containerSize < 45) return false;
        ...
        if (debugMode.get() && isCaptcha) {
            info("🔐 CAPTCHA detected: " + filledSlots + " filled slots in " + containerSize + " container slots");
        }
