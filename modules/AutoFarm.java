...
public class AutoFarm extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (pauseOnGui.get() && mc.screen != null && !isMeteorGui()) {
            return;
        }
    ...
    // Handle the shift + right-click sequence with delay
    if (shiftPressed) {
        shiftHoldTicks++;
        
        // Wait 5 ticks for shift to register
        if (shiftHoldTicks >= 5) {
            // Press shift - using setDown() for Mojang mappings
            mc.options.keyShift.setDown(true);
            
            // Right click using Meteor's Utils
            meteordevelopment.meteorclient.utils.Utils.rightClick();
    ...
    if (lastAbilityActivation == 0) {
        if (currentTime >= activationDelayMs) {
    ...
    if (timeSinceLastActivation >= timeToNextActivation) {
        startAbilitySequence();
    }
}

private void startAbilitySequence() {
    mc.options.keyShift.setDown(true);
    shiftPressed = true;
    shiftHoldTicks = 0;
}
...
public void onDeactivate() {
    ...
    // Clean up - release shift if module deactivated mid-sequence
    if (shiftPressed) {
        mc.options.keyShift.setDown(false);
        shiftPressed = false;
    }
    ...
    // Helper method to detect Meteor GUI
    private boolean isMeteorGui() {
        if (mc.screen == null) return false;
        String className = mc.screen.getClass().getName();
        return className.startsWith("meteordevelopment.meteorclient.gui");
    }
    ...
    private void registerCuboidBlocks() {
        ...
        // Calculate the bounds based on facing direction
        ...
        // Adjust bounds based on facing direction
        switch (facing) {
            case NORTH:
        ...
        // Iterate through the defined box
    ...
    private boolean harvest(BlockPos pos, BlockState state, Block block) {
        ...
        // Special handling for Sugarcane and Cactus
        if (block == Blocks.SUGAR_CANE || block == Blocks.CACTUS) {
            // Only break if the block below is the same type (i.e., it's not the bottom block)
